...
impl Task for LaunchOnExitTask {
    fn execute(
        ...
        let mut pkg = None;
        for config_package in &context
            .config
            .as_ref()
            .log_expect("Config should be loaded by now")
            .packages
        {
            if config_package.launch_after_install {
                let found_pkg = context
                    .database
                    .packages
                    .iter()
                    .find(|local_package| local_package.name == config_package.name);
                if let Some(launch_pkg) = found_pkg {
                    pkg = Some(launch_pkg);
                }
            }
        }
        if pkg.is_none() {
            pkg = context.database.packages.first();
            if pkg.is_none() {
