...
pub fn handle(service: &WebService, req: Request) -> Future {
    ...
    Box::new(req.body().concat2().map(move |b| {
        ...
        for (key, value) in &results {
            ...
            let package_name = key.to_owned();
            //target_version == None means that we target the latest version
            if value == "latest_ver" {
                to_install.insert(package_name, VersionTarget::Latest);
            } else {
                //TODO: implement parsing for SemVer Version type
                let target_version = VersionTarget::Specific(Version::Integer(
                    value
                        .to_owned()
                        .parse()
                        .expect("Unable to parse package version to int"),
                ));
                to_install.insert(package_name, target_version);
            }
        }
