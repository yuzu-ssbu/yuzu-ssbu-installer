...
const app = new Vue({
  ...
  methods: {
    get_and_cache_package_metadata: function (success, error) {
      this.$http.get('/api/installation-status').then((resp) => {
        app.metadata = resp.data
        app.package_preexisting_data = {}
        for (let i = 0; i < app.metadata.database.packages.length; i++) {
          const local_pkg = app.metadata.database.packages[i]
          const pkg_version = local_pkg.version.Integer || local_pkg.version.SemverVersion
          app.package_preexisting_data[local_pkg.name] = { latest: local_pkg.latest, version: pkg_version.toString() }
        }
        if (success) {
          success()
        }
    ...
    get_and_cache_package_versions: function (success, error) {
      app.package_versions = {}
      for (let i = 0; i < app.$data.config.packages.length; i++) {
        const pkg = app.$data.config.packages[i]
        this.$http.get(`/api/package-versions?package_name=${pkg.name}`).then((resp) => {
          const versions = resp.data || {}
          versions.unshift({ version: 'latest_ver', name: 'Latest' })
          app.package_versions[pkg.name] = versions
          if (success) {
            success()
          }
        }).catch(function () {
