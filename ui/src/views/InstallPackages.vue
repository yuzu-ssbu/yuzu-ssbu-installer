  ...
  methods: {
    install: function () {
      ...
      for (let package_index = 0; package_index < app.config.packages.length; package_index++) {
        ...
        if (current_package.default != null && current_package.default) {
          if (!current_package.source.allow_version_control) {
            results[current_package.name] = 'latest_ver'
            continue
          }
          if (current_package && current_package.selected_version) {
            results[current_package.name] = current_package.selected_version.version
          } else {
            if (this.$root.package_preexisting_data[current_package.name].latest) {
              results[current_package.name] = 'latest_ver'
            } else {
              results[current_package.name] = this.$root.package_preexisting_data[current_package.name].version
            }
          }
        }
      }
